<?php 
  ...
  $conn = mysqli_connect('localhost', 'bhargavi', 'mysqlpwd123', 'bloodbank');
    ...
    if (isset($_POST['prof_userid'])) {
      ...
      echo $userid;
      $url = "H_poc.php?prof_userid=" . $userid;
    ...
    $query = "SELECT name from hospital where hospitalid='$hid_ans' ";
    $result = mysqli_query($conn, $query);
    $hnames = mysqli_fetch_all($result, MYSQLI_ASSOC);
...
<style>
  ...
  * {
  ...
  <div class="container" style="background-color: #e5e5e5; padding-top: 10px; width: 1200px; height: 480px;margin-top: 40px;margin-bottom: 50px">
    <div class="row">
      <div class="column left" style="background-color:#e5e5e5;">
        ...
        <h2 style="font-family: 'Lucida Sans Unicode', 'Lucida Grande', sans-serif;text-align: right;padding-top: 5px">Hospital</h2>
        <form method="post" action="H_change_pass.php">
          <input type="hidden" name="change_pwd" <?php echo 'value="'.$userid.'"'  ?>>
          <input type="submit" value="Change Password" style="margin-right: 65px;">
      </form>
      </div>
      <div class="column middle" style="background-color:#e5e5e5;">
        ...
        <h2 style="text-align: center;padding-top: 5px"> : </h2>
        <h2 style="text-align: center;padding-top: 5px"> : </h2>
        <form method="post" action="H_delete_acc.php">
          <input type="hidden" name="delete" <?php echo 'value="'.$userid.'"'  ?>>
          <input type="submit" value="Delete Account" style="margin-top: 30px">
      </form>
      </div>
      <div class="column right" style="background-color:#e5e5e5;">
        ...
        <h2 style="font-family: 'Lucida Sans Unicode', 'Lucida Grande', sans-serif;text-align: left;padding-top: 5px"><?php echo $hname ?></h2>
        <form method="post" action="H_change_phoneno.php">
          <input type="hidden" name="change_phno" <?php echo 'value="'.$userid.'"'  ?>>
          <input type="submit" value="Change Phone Number" style="margin-left: 65px;">
      </form>
      </div>
  </div>
  </div>
...
<p style="text-align: center;">
  <form method="post" action="H_profile.php">
    <input type="hidden" name="prof_userid" <?php echo 'value="'.$userid.'"'  ?>>
    <input type="image" src="img1/home.png" alt="submit" style="margin-left: 690px">
  </form>
  </p>
