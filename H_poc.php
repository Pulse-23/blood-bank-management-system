  ...
  $conn = mysqli_connect('localhost', 'bhargavi', 'mysqlpwd123', 'bloodbank');
    ...
    if(isset($_POST['submit']) or isset($_POST['signin'])) {
      ...
      if(empty($userid_username)){
        header("Location: H_signin_failure.php");
      }
      $userid_username_ans = $userid_username[0]['userid'];

      $query_userid_pwd = " SELECT userid FROM users where pwd='$pwd' and role='HPOC' ";
    ...
    if (isset($_POST['userid'])) {
      $userid = $_POST['userid'];
      $url = "H_profile.php?userid=" . $userid;
      header((string)'Location: '.$url);
      exit();
    }


    if (isset($_POST['demand_userid'])) {
      $userid = $_POST['demand_userid'];
      $url = "H_demand_details.php?demand_userid=" . $userid;
      header((string)'Location: '.$url);
      exit();
    }
    ...
    if (isset($_POST['view_userid'])) {
      ...
      $url = "H_view_demand.php?view_userid=" . $userid;
      header((string)'Location: '.$url);
      exit();
    }
...
<div class="row">
  <div class="column">
    <p style="text-align: center;">
      <form method="post" action="H_poc.php">
        <input type="hidden" name="demand_userid" <?php echo 'value="'.$userid.'"'  ?>>
        <input type="image" src="img1/poc4.png" alt="submit">
      </form>
    </p>
